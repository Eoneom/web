...
export const getReport = createAppAsyncThunk('report/get', async (reportId: string, { getState, rejectWithValue, dispatch }) => {
  const token = selectToken(getState())
  if (!token) {
    throw rejectWithValue('empty token')
  }
...
export const markReportAsRead = createAppAsyncThunk('report/mark-read', async (reportId: string, { getState, rejectWithValue, dispatch }) => {
  const token = selectToken(getState())
  if (!token) {
    throw rejectWithValue('empty token')
  }
